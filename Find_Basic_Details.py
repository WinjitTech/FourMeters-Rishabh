...
def get_cardianls_from_image(meter_no, img_path, supression, min_range, max_range, iterate, size):
    meter_info = open(img_path+"\\Auto\\MeterImages\\Crop\\"+str(meter_no)+"\\"+str(meter_no)+".json", "w")
    meterinfo = {}
    ...
    img, cardinal_contour_list, cardinal_coordinates = GetCardinalContours.draw_main_cardinals(img_path, contour_array, top_x,
                                                                                               base_y, min_dist,
                                                                                               min_range, max_range,
                                                                                               meter_no, size)
    ...
    cv2.waitKey(0)
    cv2.imwrite(img_path + "\\Auto\\MeterImages\\Crop\\"+str(meter_no)+"\\"+"cardinals.jpg", img)
